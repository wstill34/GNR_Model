...
simulate_maternal <- function (birth_cohort, periods, h, mu_ac, mu_gnr, n_simulations, vm, vm_se, em, em_se) {
...
for (sim in 1:n_simulations) {
  ...
  vaxed_population <- round(remaining_cohort*vm_sample_mat[sim])
  unvax_population <- round(remaining_cohort*(1 - vm_sample_mat[sim]))
  ...
  for (age in 1:periods) {
    if (age == 1) {
      ...
      hospitalizations_unvax_sim_mat[sim, age] <- rbinom(1, unvax_population, h_vec_sim_mat[sim,age]) #for each age group, estimates number of hospitalized babies based on h_vec_sim and binomial distribution
      hospitalizations_vaxed_sim_mat[sim, age] <- rbinom(1, vaxed_population, h_vec_sim_mat[sim, age]*(1 - em_sample_mat[sim]))
      ...
      unvax_deaths_due_to_gnr_sim_mat[sim, age] <- rbinom(1, hospitalizations_unvax_sim_mat[sim, age], mu_gnr_sim_mat[sim,age])
      vaxed_deaths_due_to_gnr_sim_mat[sim, age] <- rbinom(1, hospitalizations_vaxed_sim_mat[sim, age], mu_gnr_sim_mat[sim,age])
      ...
      h_vec_sim_mat[sim, age] <- rbeta(1, h_vec[age], h_n[age]-h_vec[age]) #For each age group, estimates a hospitalization probability based on the mean and standard deviation and 1 trial
      hospitalizations_total_sim_mat[sim, age] <- rbinom(1, remaining_cohort, h_vec_sim_mat[sim,age]) #for each age group, estimates number of hospitalized babies based on h_vec_sim and binomial distribution
      mu_gnr_sim_mat[sim, age] <- rbeta(1, mu_gnr_vec[age], mu_gnr_n[age] - mu_gnr_vec[age])
      total_deaths_due_to_gnr_sim_mat[sim, age] <- rbinom(1, hospitalizations_total_sim_mat[sim, age], mu_gnr_sim_mat[sim,age])
